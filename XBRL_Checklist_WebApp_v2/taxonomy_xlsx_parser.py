...
def _parse_basic_info(df, data: TaxonomyXlsxData):
    for _, row in df.iterrows():
        for j, cell in enumerate(row):
            s = _safe(cell)
            if ('회사명' in s or '법인명' in s) and j+1 < len(row):
                v = _safe(row.iloc[j+1])
                if v: data.company_name = v
            if ('문서작성일' in s or '회계기간종료일' in s) and j+1 < len(row):
                v = _safe(row.iloc[j+1])
                if v and not data.report_date: data.report_date = v